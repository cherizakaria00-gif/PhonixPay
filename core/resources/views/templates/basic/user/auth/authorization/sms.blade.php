...
<div class="new-auth min-h-screen bg-slate-950 text-white relative overflow-hidden">
    ...
    <div class="relative z-10 min-h-screen flex items-center justify-center px-6 pb-12 pt-24 lg:pt-28">
        <div class="w-full flex items-center justify-center">
            <div class="w-full max-w-md bg-slate-900/70 border border-slate-800 rounded-2xl p-8 shadow-2xl">
                <h2 class="text-2xl font-bold text-white mb-2">@lang('Verify Mobile Number')</h2>
                <p class="text-slate-400 mb-6">
                    @lang('A 6 digit verification code sent to your mobile number') :
                    <span class="text-indigo-300">+{{ showMobileNumber(auth()->user()->mobile) }}</span>
                </p>

                <form action="{{route('user.verify.mobile')}}" method="POST" class="submit-form space-y-5">
                    @csrf
                    ...
                    <div class="text-sm text-slate-400">
                        ...
                        <a href="{{route('user.send.verify.code', 'sms')}}" class="try-again-link d-none anchor-color"> @lang('Try again')</a>
                    </div>
