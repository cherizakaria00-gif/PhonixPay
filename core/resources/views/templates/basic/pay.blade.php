...
<div class="min-h-screen bg-[#020618] text-white font-sans selection:bg-[#87c5a6]/30">
    ...
    <main>
        ...
        <section id="pricing" class="bg-[#040b1e] py-16 sm:py-24 relative overflow-hidden">
                <div class="absolute inset-0 bg-[radial-gradient(circle_at_20%_15%,rgba(56,189,248,0.12),transparent_35%),radial-gradient(circle_at_80%_10%,rgba(251,191,36,0.12),transparent_35%),radial-gradient(circle_at_50%_100%,rgba(135,197,166,0.14),transparent_45%)]"></div>
                <div class="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                    <div class="text-center">
                        <div class="inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/5 px-4 py-2 text-xs text-slate-300">
                            <span class="h-2 w-2 rounded-full bg-[#87c5a6]"></span>
                            Secure & Trusted Pricing
                        </div>
                        <h2 class="mt-5 text-4xl sm:text-5xl font-bold text-white">
                            Choose Your <span class="text-[#87c5a6]">Perfect Plan</span>
                        </h2>
                        <p class="mt-3 text-slate-300 text-lg">Simple, transparent pricing. Upgrade anytime.</p>

                        <div class="mt-6 inline-flex items-center gap-2 rounded-full border border-white/15 bg-black/40 p-1 text-sm">
                            <span class="rounded-full bg-[#87c5a6] px-4 py-2 font-semibold text-slate-900">Monthly Billing</span>
                            <span class="px-4 py-2 text-slate-300">Yearly Billing</span>
                            <span class="rounded-full bg-emerald-500/20 px-3 py-1 text-xs font-semibold text-emerald-300">Save 20%</span>
                        </div>
                    </div>
                    ...
                    <div class="mt-10 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
                        @foreach($pricingPlans as $plan)
                            @php
                                $styleMap = [
                                    'starter' => ['glow' => 'rgba(16,185,129,.42)', 'border' => 'rgba(16,185,129,.65)', 'button' => 'from-emerald-400 to-emerald-500', 'tag' => 'Free'],
                                    'growth' => ['glow' => 'rgba(59,130,246,.45)', 'border' => 'rgba(56,189,248,.75)', 'button' => 'from-sky-400 to-blue-500', 'tag' => 'Most Popular'],
                                    'pro' => ['glow' => 'rgba(139,92,246,.45)', 'border' => 'rgba(168,85,247,.7)', 'button' => 'from-violet-500 to-purple-600', 'tag' => 'Best Value'],
                                    'business' => ['glow' => 'rgba(245,158,11,.48)', 'border' => 'rgba(245,158,11,.8)', 'button' => 'from-amber-400 to-yellow-500', 'tag' => 'Unlimited'],
                                ];
                                ...
                                $notifyText = in_array('sms', $plan->notification_channels ?? []) ? 'Push + SMS Notifications' : 'Push Notifications';
                                $paymentLinksEnabled = (bool)($planFeatures['payment_links'] ?? false);
                                $cta = match($plan->slug) {
                            ...
                            @endphp
                            <article
                                class="relative rounded-3xl border p-6 bg-[#060e24]/90 backdrop-blur-md"
                                style="border-color: {{ $style['border'] }}; box-shadow: 0 0 40px {{ $style['glow'] }};"
                            >
                                <div class="flex items-center justify-between">
                                    <h3 class="text-3xl font-extrabold text-white">{{ $plan->name }}</h3>
                                    <span class="rounded-full bg-white/10 px-3 py-1 text-xs font-semibold text-white">{{ $style['tag'] }}</span>
                                </div>

                                <p class="mt-4 text-6xl font-black text-white leading-none">
                                    ${{ number_format($plan->price_monthly_cents / 100, 0) }}
                                    <span class="text-xl font-semibold text-slate-300">/month</span>
                                </p>

                                <ul class="mt-6 space-y-3 text-[17px] text-slate-100">
                                    <li class="flex items-start gap-2"><span class="text-[#87c5a6]">✓</span> {{ $txLimit }}</li>
                                    <li class="flex items-start gap-2"><span class="text-[#87c5a6]">✓</span> {{ number_format($plan->fee_percent, 2) }}% + ${{ number_format($plan->fee_fixed, 2) }} Fees</li>
                                    <li class="flex items-start gap-2"><span class="text-[#87c5a6]">✓</span> {{ $payoutLabel }}</li>
                                    <li class="flex items-start gap-2"><span class="text-[#87c5a6]">✓</span> {{ $supportText }}</li>
                                    <li class="flex items-start gap-2"><span class="text-[#87c5a6]">✓</span> {{ $notifyText }}</li>
                                    <li class="flex items-start gap-2">
                                        @if($paymentLinksEnabled)
                                            <span class="text-[#87c5a6]">✓</span> Payment Links
                                        @else
                                            <span class="text-rose-400">✕</span> Payment Links
                                        @endif
                                    </li>
                                </ul>

                                <a href="{{ route('user.register') }}"
                                   class="mt-7 inline-flex w-full items-center justify-center rounded-full bg-gradient-to-r {{ $style['button'] }} px-5 py-3 text-lg font-extrabold text-slate-950 shadow-lg">
                                    {{ $cta }}
                                </a>
                            </article>
                        @endforeach
                    ...
                    <div class="mt-8 flex flex-wrap items-center justify-center gap-3 text-sm">
                        <span class="rounded-full border border-white/20 bg-white/5 px-4 py-2 text-slate-200">Secure Checkout</span>
                        <span class="rounded-full border border-white/20 bg-white/5 px-4 py-2 text-slate-200">Fraud Protection</span>
                        <span class="rounded-full border border-white/20 bg-white/5 px-4 py-2 text-slate-200">Analytics Dashboard</span>
                        <span class="rounded-full border border-white/20 bg-white/5 px-4 py-2 text-slate-200">API Access</span>
                        <span class="rounded-full border border-white/20 bg-white/5 px-4 py-2 text-slate-200">Instant Activation</span>
                    </div>

                    <p class="mt-5 text-center text-xs text-slate-400">Fees apply per successful transaction.</p>
                </div>
            </section>
