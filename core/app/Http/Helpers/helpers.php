...
function paymentRedirectUrl($apiPayment, $success = true, $reason = null)
{   
    if($success){
        return $apiPayment->success_url;
    }

    if($apiPayment->status != Status::PAYMENT_SUCCESS){
        $apiPayment->status = Status::PAYMENT_CANCEL;
        $apiPayment->cancel_reason = $reason;
        $apiPayment->save();
    }
